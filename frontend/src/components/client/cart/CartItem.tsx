...
const CartItem: React.FC<CartItemProps> = ({ id, images, title, newPrice, quantity, isChecked, onRemove, onUpdateQuantity, onToggleSelect }) => {
  ...
  return (
    <div className="flex sm:grid sm:grid-cols-12 gap-2 sm:gap-4 pb-2 sm:pb-4 border-b border-gray-300">
      ...
      <div className="col-span-5 flex flex-wrap sm:flex-nowrap items-center gap-2 sm:gap-4">
        ...
        />
        <Image src={thumbnail} alt={title} width={48} height={48} className="w-10 h-12 sm:w-20 sm:h-20 rounded-md" />
        <h3 className="text-sm sm:text-lg font-semibold">{title}</h3>
      </div>

      {/* Cột 2: Giá */}
      <div className="col-span-2 flex items-center justify-center text-red-500 font-bold text-sm sm:text-base">{formattedPrice}</div>

      {/* Cột 3: Điều chỉnh số lượng */}
      <div className="col-span-2 flex items-center justify-center gap-1 sm:gap-2">
        <Button onClick={() => handleQuantityChange(-1)} className=" p-1.5  border rounded text-black bg-gray-100 hover:bg-gray-200">-</Button>
        <p className="text-sm sm:text-base">{itemQuantity}</p>
        <Button onClick={() => handleQuantityChange(1)} className="  p-1 border rounded text-black bg-gray-100 hover:bg-gray-200">+</Button>
      </div>

      {/* Cột 4: Tổng giá */}
      <p className="col-span-2 hidden sm:flex items-center justify-center text-sm sm:text-lg font-semibold">{formattedSubtotal}</p>
